...
void ordenabarcos(Board *board)
{
    Board tmp;
    for (int i = 0; i < N; i++)
        for (int j = i + 1; j < M; j++)
        {
            if (typeToSize(barconoarray(i,j,board))<typeToSize(barconoarray(i,j,board)))
            {
                tmp.boats[0] = board->boats[i];
                board->boats[i] = board->boats[j];
                board->boats[j] = tmp.boats[0];
            }
        }
}
/**
